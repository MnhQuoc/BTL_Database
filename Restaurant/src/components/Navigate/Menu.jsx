...
const Menu = () => {
  ...
  useEffect(() => {
    ...
    const fetchAdminData = async () => {
      try {
        ...
        // Fetch menu (thay thế tutorCourses)
        let tutorCoursesData = [];
        let coursesData = [];
        try {
          const menuRes = await axios.get('http://localhost:3001/menu');
          tutorCoursesData = menuRes.data || [];
        } catch (err) {
          console.warn('menu endpoint not found, trying courses...');
        }
        
        // Also fetch courses to get all staff (for dropdown)
        try {
        ...
        // If menu is empty or has less data, use courses as fallback
  ...
  const getStudentName = (tutorCourse) => {
    const staffId = tutorCourse?.staffId || tutorCourse?.tutorId;
    if (!tutorCourse || !staffId) return '—';
    
    // Find order that matches this menu item
    const matchingOrder = orders.find(order => 
      order.tutorId === staffId.toString() &&
  ...
  // Get list of staff from courses (to show all available staff)
  const tutors = useMemo(() => {
    // Return all staff with role 'staff' - show all staff, not just those with courses
  ...
  const tutorClasses = useMemo(() => {
    ...
    return tutorCourses.filter(course => {
      const staffId = course.staffId || course.tutorId;
      return staffId && staffId.toString() === selectedTutorId;
    });
  ...
  const handleTutorChange = (tutorId) => {
    setSelectedTutorId(tutorId);
    const classes = tutorCourses.filter(course => {
      const staffId = course.staffId || course.tutorId;
      return staffId === tutorId;
    });
    if (classes.length > 0) {
